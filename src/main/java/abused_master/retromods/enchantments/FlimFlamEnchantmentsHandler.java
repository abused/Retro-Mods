package abused_master.retromods.enchantments;
...
public class FlimFlamEnchantmentsHandler {
	...
	private static final ResourceLocation CAPABILITY_KEY = RetroMods.location("luck");
	...
	private static int getFlimFlamToolLevel(EntityPlayer player) {
		return EnchantmentHelper.getEnchantmentLevel(RetroMods.Enchantments.flimFlam, player.getHeldItemMainhand());
	}

	private static int getFlimFlamArmorLevel(EntityPlayer player) {
		int sum = 0;
		for (ItemStack stack : player.inventory.armorInventory) {
			sum += EnchantmentHelper.getEnchantmentLevel(RetroMods.Enchantments.flimFlam, stack);
		}
		return sum;
	}
}
